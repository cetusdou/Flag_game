import requests
from PIL import Image, ImageEnhance, ImageOps
from io import BytesIO
import math
import os
import json
import time

# --- ⚙️ 配置 ---
IMG_DIR = "./assets/city_networks"
DATA_DIR = "./data"
os.makedirs(IMG_DIR, exist_ok=True)
os.makedirs(DATA_DIR, exist_ok=True)

# 🇨🇳 155 城终极名单 (ID, 中文名, 纬度, 经度, 缩放)
# 缩放: 12=大城市全貌, 13=中等城市/细节, 14=小城/特色纹理
targets = [
    # --- 原有的 100 个基础 (直辖/省会/副省级/热门) ---
    ("beijing", "北京", 39.9042, 116.4074, 12),
    ("shanghai", "上海", 31.2304, 121.4737, 12),
    ("tianjin", "天津", 39.0842, 117.2009, 12),
    ("chongqing", "重庆", 29.5630, 106.5516, 13),
    ("hong_kong", "香港", 22.3193, 114.1694, 13),
    ("macau", "澳门", 22.1987, 113.5439, 14),
    ("taipei", "台北", 25.0330, 121.5654, 13),
    ("guangzhou", "广州", 23.1291, 113.2644, 12),
    ("shenzhen", "深圳", 22.5431, 114.0579, 12),
    ("zhuhai", "珠海", 22.2707, 113.5767, 13),
    ("foshan", "佛山", 23.0215, 113.1214, 13),
    ("dongguan", "东莞", 23.0205, 113.7518, 12),
    ("shantou", "汕头", 23.3517, 116.6787, 13),
    ("haikou", "海口", 20.0440, 110.1999, 13),
    ("sanya", "三亚", 18.2528, 109.5120, 13),
    ("nanning", "南宁", 22.8170, 108.3665, 12),
    ("guilin", "桂林", 25.2345, 110.1800, 13),
    ("liuzhou", "柳州", 24.3255, 109.4126, 13),
    ("beihai", "北海", 21.4812, 109.1192, 13),
    ("nanjing", "南京", 32.0603, 118.7969, 12),
    ("suzhou", "苏州", 31.2989, 120.5853, 13),
    ("wuxi", "无锡", 31.4912, 120.3119, 13),
    ("changzhou", "常州", 31.8112, 119.9741, 13),
    ("yangzhou", "扬州", 32.3942, 119.4129, 13),
    ("xuzhou", "徐州", 34.2048, 117.2841, 13),
    ("hangzhou", "杭州", 30.2741, 120.1551, 12),
    ("ningbo", "宁波", 29.8683, 121.5440, 12),
    ("wenzhou", "温州", 27.9943, 120.6994, 13),
    ("shaoxing", "绍兴", 30.0024, 120.5861, 13),
    ("jinhua", "金华", 29.0790, 119.6474, 13),
    ("yiwu", "义乌", 29.3003, 120.0751, 13),
    ("hefei", "合肥", 31.8206, 117.2272, 12),
    ("fuzhou", "福州", 26.0745, 119.2965, 12),
    ("xiamen", "厦门", 24.4798, 118.0894, 13),
    ("quanzhou", "泉州", 24.8741, 118.6757, 13),
    ("nanchang", "南昌", 28.6820, 115.8579, 12),
    ("jingdezhen", "景德镇", 29.2690, 117.1782, 13),
    ("jinan", "济南", 36.6512, 117.1201, 12),
    ("qingdao", "青岛", 36.0671, 120.3826, 13),
    ("yantai", "烟台", 37.4638, 121.4479, 13),
    ("weifang", "潍坊", 36.7072, 119.1617, 13),
    ("zibo", "淄博", 36.8135, 118.0550, 13),
    ("kaohsiung", "高雄", 22.6273, 120.3014, 13),
    ("taichung", "台中", 24.1477, 120.6736, 13),
    ("tainan", "台南", 22.9997, 120.2270, 13),
    ("wuhan", "武汉", 30.5928, 114.3055, 12),
    ("yichang", "宜昌", 30.6920, 111.2865, 13),
    ("xiangyang", "襄阳", 32.0087, 112.1224, 13),
    ("changsha", "长沙", 28.2282, 112.9388, 12),
    ("zhuzhou", "株洲", 27.8274, 113.1338, 13),
    ("yueyang", "岳阳", 29.3567, 113.1289, 13),
    ("zhangjiajie", "张家界", 29.1170, 110.4792, 14),
    ("zhengzhou", "郑州", 34.7466, 113.6253, 12),
    ("luoyang", "洛阳", 34.6181, 112.4540, 13),
    ("kaifeng", "开封", 34.7972, 114.3076, 13),
    ("shijiazhuang", "石家庄", 38.0428, 114.5149, 12),
    ("tangshan", "唐山", 39.6301, 118.1802, 13),
    ("qinhuangdao", "秦皇岛", 39.9617, 119.6005, 13),
    ("taiyuan", "太原", 37.8706, 112.5489, 12),
    ("datong", "大同", 40.0768, 113.3001, 13),
    ("hohhot", "呼和浩特", 40.8415, 111.7492, 12),
    ("baotou", "包头", 40.6579, 109.8404, 13),
    ("shenyang", "沈阳", 41.8057, 123.4315, 12),
    ("dalian", "大连", 38.9140, 121.6147, 13),
    ("changchun", "长春", 43.8171, 125.3235, 12),
    ("harbin", "哈尔滨", 45.8038, 126.5350, 12),
    ("daqing", "大庆", 46.5845, 125.1037, 13),
    ("qiqihar", "齐齐哈尔", 47.3543, 123.9182, 13),
    ("jilin", "吉林市", 43.8378, 126.5496, 13),
    ("chengdu", "成都", 30.5728, 104.0668, 12),
    ("leshan", "乐山", 29.5521, 103.7656, 13),
    ("kunming", "昆明", 24.8801, 102.8329, 12),
    ("dali", "大理", 25.6065, 100.2676, 13),
    ("lijiang", "丽江", 26.8550, 100.2277, 14),
    ("jinghong", "景洪(版纳)", 22.0017, 100.7979, 14),
    ("guiyang", "贵阳", 26.6470, 106.6302, 13),
    ("zunyi", "遵义", 27.7050, 106.9270, 13),
    ("lhasa", "拉萨", 29.6525, 91.1721, 13),
    ("shigatse", "日喀则", 29.2675, 88.8752, 14),
    ("xi_an", "西安", 34.3416, 108.9398, 12),
    ("baoji", "宝鸡", 34.3619, 107.2373, 13),
    ("yanan", "延安", 36.5854, 109.4897, 14),
    ("lanzhou", "兰州", 36.0611, 103.8343, 12),
    ("dunhuang", "敦煌", 40.1421, 94.6620, 14),
    ("xining", "西宁", 36.6171, 101.7782, 13),
    ("yinchuan", "银川", 38.4872, 106.2309, 12),
    ("zhongwei", "中卫", 37.5136, 105.1896, 14),
    ("urumqi", "乌鲁木齐", 43.8256, 87.6168, 12),
    ("kashgar", "喀什", 39.4704, 75.9898, 14),
    ("turpan", "吐鲁番", 42.9333, 89.1897, 14),
    ("korla", "库尔勒", 41.7641, 86.1453, 13),
    ("karamay", "克拉玛依", 45.6032, 84.8694, 13),
    ("tekes", "特克斯", 43.2146, 81.8412, 15),
    ("shihezi", "石河子", 44.3054, 86.0332, 13),

    # --- 🔥 新增 50+ 城市 (华东/华南/西南/西北/东北) ---
    # 江浙富庶地
    ("jiaxing", "嘉兴", 30.7539, 120.7522, 13), # 南湖
    ("huzhou", "湖州", 30.8930, 120.0873, 13), # 太湖边
    ("taizhou_zj", "台州", 28.6564, 121.4208, 13),
    ("zhenjiang", "镇江", 32.1878, 119.4252, 13), # 长江
    ("taizhou_js", "泰州", 32.4555, 119.9246, 13),
    ("nantong", "南通", 31.9802, 120.8943, 13),
    ("wuhu", "芜湖", 31.3528, 118.3755, 13),
    ("anqing", "安庆", 30.5248, 117.0428, 13),
    
    # 山东大汉
    ("jining", "济宁", 35.4149, 116.5872, 13),
    ("weihai", "威海", 37.5130, 122.1204, 13), # 海滨
    ("linyi", "临沂", 35.0518, 118.3119, 13),
    
    # 华南/江西
    ("ganzhou", "赣州", 25.8311, 114.9347, 13), # 宋城
    ("zhanjiang", "湛江", 21.2707, 110.3594, 13), # 军港
    ("huizhou", "惠州", 23.1115, 114.4161, 13), # 西湖
    ("zhongshan", "中山", 22.5170, 113.3928, 13),
    ("jiangmen", "江门", 22.5787, 113.0819, 13), # 侨乡
    
    # 华中
    ("hengyang", "衡阳", 26.8968, 112.5725, 13),
    ("changde", "常德", 29.0317, 111.6985, 13), # 桃花源
    ("jingzhou", "荆州", 30.3323, 112.2354, 13), # 古城墙
    ("shiyan", "十堰", 32.6475, 110.7993, 13), # 车城
    
    # 华北
    ("handan", "邯郸", 36.6256, 114.5391, 13),
    ("baoding", "保定", 38.8738, 115.4648, 13),
    ("zhangjiakou", "张家界", 40.8183, 114.8859, 13), # 冬奥
    ("chengde", "承德", 40.9762, 117.9624, 13), # 避暑山庄
    ("ordos", "鄂尔多斯", 39.6083, 109.7816, 13), # 康巴什鬼城(规划整齐)
    
    # 东北
    ("anshan", "鞍山", 41.1075, 122.9944, 13), # 钢都
    ("dandong", "丹东", 40.1242, 124.3830, 13), # 边境
    ("jinzhou", "锦州", 41.0951, 121.1270, 13),
    ("yanji", "延吉", 42.9068, 129.5076, 13), # 朝鲜族特色
    
    # 西北
    ("xianyang", "咸阳", 34.3296, 108.7089, 13),
    ("yulin_sx", "榆林", 38.2853, 109.7347, 13), # 陕北
    ("tianshui", "天水", 34.5808, 105.7249, 13), # 麦积山
    ("jiuquan", "酒泉", 39.7321, 98.4942, 13), # 卫星
    ("hami", "哈密", 42.8185, 93.5151, 13),
    ("aksu", "阿克苏", 41.1687, 80.2606, 13),
    
    # 西南
    ("mianyang", "绵阳", 31.4674, 104.7578, 13), # 科技城
    ("yibin", "宜宾", 28.7525, 104.6432, 13), # 酒都
    ("nanchong", "南充", 30.7991, 106.0829, 13),
    ("bijie", "毕节", 27.3019, 105.2863, 13),
    ("shangrila", "香格里拉", 27.8288, 99.7072, 14), # 独克宗
    ("panzhihua", "攀枝花", 26.5823, 101.7186, 13), # 山谷城市
    ("luzhou", "泸州", 28.8724, 105.4404, 13),
    ("deyang", "德阳", 31.1279, 104.3986, 13),
    ("guangyuan", "广元", 32.4417, 105.8433, 13),
    ("dazhou", "达州", 31.2096, 107.4680, 13),
    ("liupanshui", "六盘水", 26.5926, 104.8302, 13), # 凉都
    ("anshun", "安顺", 26.2530, 105.9462, 13), # 黄果树
    ("tongren", "铜仁", 27.7172, 109.1899, 13),
    ("qujing", "曲靖", 25.4899, 103.7978, 13),
    ("yuxi", "玉溪", 24.3520, 102.5439, 13),
    ("hanzhong", "汉中", 33.0676, 107.0236, 13),
    ("jiayuguan", "嘉峪关", 39.7731, 98.2891, 14), # 关城
    ("zhangye", "张掖", 38.9259, 100.4498, 13), # 七彩丹霞
    ("hotan", "和田", 37.1141, 79.9222, 13),
    ("yining", "伊宁", 43.9143, 81.3179, 13), # 塞外江南
    ("dezhou", "德州", 37.4340, 116.3574, 13),
]

# CartoDB Dark No-Labels 源
TILE_URL = "https://a.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}.png"

def latlon_to_tile(lat, lon, zoom):
    n = 2.0 ** zoom
    xtile = int((lon + 180.0) / 360.0 * n)
    ytile = int((1.0 - math.log(math.tan(math.radians(lat)) + (1 / math.cos(math.radians(lat)))) / math.pi) / 2.0 * n)
    return xtile, ytile

def process_image(img):
    img = img.convert("L")
    img = ImageEnhance.Contrast(img).enhance(5.0) 
    img = ImageEnhance.Brightness(img).enhance(1.6)
    img = ImageOps.colorize(img, black="black", white="white")
    return img

def download_merged_image(name_id, lat, lon, zoom):
    center_x, center_y = latlon_to_tile(lat, lon, zoom)
    range_offset = 1 
    tile_size = 256
    width = tile_size * (range_offset * 2 + 1)
    height = tile_size * (range_offset * 2 + 1)
    
    merged_img = Image.new('RGB', (width, height))
    
    print(f"🖼️ [{zoom}] 拼图: {name_id} ...", end="")
    
    has_error = False
    
    for dx in range(-range_offset, range_offset + 1):
        for dy in range(-range_offset, range_offset + 1):
            xtile = center_x + dx
            ytile = center_y + dy
            
            url = TILE_URL.format(z=zoom, x=xtile, y=ytile)
            
            # 重试机制
            success = False
            for attempt in range(3):
                try:
                    headers = {'User-Agent': 'Mozilla/5.0'}
                    r = requests.get(url, headers=headers, timeout=10)
                    if r.status_code == 200:
                        tile_img = Image.open(BytesIO(r.content))
                        px = (dx + range_offset) * tile_size
                        py = (dy + range_offset) * tile_size
                        merged_img.paste(tile_img, (px, py))
                        success = True
                        break
                except:
                    time.sleep(0.5)
            
            if not success:
                print("x", end="")
                has_error = True

    final_img = process_image(merged_img)
    save_path = f"{IMG_DIR}/{name_id}.png"
    final_img.save(save_path)
    
    if has_error: print(" ⚠️ 缺")
    else: print(" ✅")

# --- 主程序 ---
print(f"🚀 启动 155 城中华路网构建 (增量版)...")

json_data = []

for i, item in enumerate(targets):
    cid, name, lat, lon, zoom = item
    
    if os.path.exists(f"{IMG_DIR}/{cid}.png"):
        print(f"⏩ 跳过: {name}")
    else:
        download_merged_image(cid, lat, lon, zoom)
    
    json_data.append({
        "id": cid,
        "name": name,
        "img": f"./assets/city_networks/{cid}.png",
        'pos': [lat, lon]
    })

# 覆盖保存 json
with open(f"{DATA_DIR}/china_city_networks.json", "w", encoding='utf-8') as f:
    json.dump(json_data, f, ensure_ascii=False, indent=2)

print(f"\n🎉 155 城路网图全部就绪！")
print(f"📂 数据文件: {DATA_DIR}/china_city_networks.json")